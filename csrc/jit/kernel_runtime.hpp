...
struct LaunchArgs {
    ...
    int num_threads;
    std::pair<int, int> block_dim;
    int smem_size;
    int cluster_dim;

    LaunchArgs(const int& grid_dim_x, const int& num_threads, const int& smem_size = 0, const int& cluster_dim = 1):
        grid_dim({grid_dim_x, 1}), num_threads(num_threads),block_dim({num_threads,1}), smem_size(smem_size), cluster_dim(cluster_dim) {}

    LaunchArgs(const std::pair<int, int>& grid_dim, const int& num_threads, const int& smem_size = 0, const int& cluster_dim = 1):
        grid_dim(grid_dim), num_threads(num_threads),block_dim({num_threads,1}), smem_size(smem_size), cluster_dim(cluster_dim) {}

    LaunchArgs(const std::pair<int, int>& grid_dim, std::pair<int, int>& block_dim, const int& smem_size = 0, const int& cluster_dim = 1):
    grid_dim(grid_dim), num_threads(block_dim.first * block_dim.second),block_dim(block_dim), smem_size(smem_size), cluster_dim(cluster_dim) {}
};
...
public:
    ...
    ~KernelRuntime() noexcept(false) {
        ...
        MO_HOST_ASSERT(error == cudaSuccess or error == cudaErrorCudartUnloading);
    }
};
...
public:
    ...
    static void launch(const std::shared_ptr<KernelRuntime>& kernel_runtime, const Args& args) {
        ...
        if (launch_args.smem_size > 0)
            MO_CUDA_RUNTIME_CHECK(cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, launch_args.smem_size));
        ...
        config.gridDim = {static_cast<unsigned>(launch_args.grid_dim.first),
                          ...
                          1};
        config.blockDim = {static_cast<unsigned>(launch_args.block_dim.first),
                           static_cast<unsigned>(launch_args.block_dim.second),
                          1};

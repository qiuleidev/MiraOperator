...
class Compiler {
    ...
    //Set version by content hash rather than version number
    //when content is changed,complier can auto complie again.
    std::string get_library_version() const {
        // Recursively walk through all subdirectories and update hash
        std::stringstream ss;
        for (const auto& entry: std::filesystem::recursive_directory_iterator(library_include_path / "MiraOperator")) {
...
public:
    ...
    explicit Compiler(const std::filesystem::path& library_root_path) {
        ...
        // Cache settings
        cache_dir_path = std::filesystem::path(get_env<std::string>("HOME")) / ".MiraOperator";
        if (const auto& env_cache_dir_path = get_env<std::string>("MO_JIT_CACHE_DIR"); not env_cache_dir_path.empty())
        ...
        if (get_env<int>("MO_JIT_PTXAS_VERBOSE", 0))
            ptxas_flags += ",--verbose";
    ...
    void put(const std::filesystem::path& path, const std::string& data) const {
        ...
        MO_HOST_ASSERT(out.write(data.data(), data.size()));
        out.close();
    ...
    std::shared_ptr<KernelRuntime> build(const std::string& name, const std::string& code) const {
        ...
        MO_HOST_ASSERT(runtime != nullptr);
        return runtime;
    }
...
class NVCCCompiler final: public Compiler {
    ...
    std::pair<int, int> get_nvcc_version() const {
        MO_HOST_ASSERT(std::filesystem::exists(nvcc_path));

        // Call the version command
        const auto& command = std::string(nvcc_path) + " --version";
        const auto& [return_code, output] = call_external_command(command);
        MO_HOST_ASSERT(return_code == 0);

        // The version should be at least 12.3, for the best performance with 12.9
        int major, minor;
        std::smatch match;
        MO_HOST_ASSERT(std::regex_search(output, match, std::regex(R"(release (\d+\.\d+))")));
        std::sscanf(match[1].str().c_str(), "%d.%d", &major, &minor);
        MO_HOST_ASSERT((major > 12 or (major == 12 and minor >= 3)) and "NVCC version should be >= 12.3");
...
public:
    NVCCCompiler(const std::filesystem::path& library_root_path,
        ...
        if (const auto& env_nvcc_path = get_env<std::string>("MO_JIT_NVCC_COMPILER"); not env_nvcc_path.empty())
    ...
    void compile(const std::string &code, const std::filesystem::path& dir_path, const std::filesystem::path &cubin_path) const override {
        ...
        if (get_env("MO_JIT_DEBUG", 0) or get_env("MO_JIT_PRINT_COMPILER_COMMAND", 0))
            printf("Running NVCC command: %s", command.c_str());
        const auto& [return_code, output] = call_external_command(command);
        if (return_code != 0) {
            printf("NVCC compilation failed: %s", output.c_str());
            MO_HOST_ASSERT(false and "NVCC compilation failed");
        }

        // Print PTXAS log
        if (get_env("MO_JIT_DEBUG", 0) or get_env("MO_JIT_PTXAS_VERBOSE", 0))

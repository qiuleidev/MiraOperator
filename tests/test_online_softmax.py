...
def test_online_softmax_bench():
    ...
    print("=== Testing Online Softmax Performance ===")
    
    print("\n--- FP32 Test ---")
    input_fp32 = torch.randn(size, device='cuda', dtype=torch.float32)
    output_fp32 = torch.zeros_like(input_fp32)
    
    def run_online_softmax_fp32():
        online_softmax(input_fp32, output_fp32)
        torch.cuda.synchronize()
    
    kernel_name = "online_soft_max_f32_per_token" 
    try:
        avg_time = bench_kineto(
            fn=run_online_softmax_fp32,
            kernel_names=kernel_name,
            num_tests=num_tests,
            suppress_kineto_output=False,
            trace_path="online_softmax_fp32_trace.json"
        )
        print(f"Custom FP32 Online Softmax Average Time: {avg_time:.6f} seconds")
...
def test_online_softmax_correctness():
    """测试在线softmax操作的正确性"""
    print("\n=== Testing Online Softmax Correctness ===")
    size = 2048
...
if __name__ == "__main__":
    test_online_softmax_correctness()
